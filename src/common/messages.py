...
class MoveMsg(Msg):
    ...
    - **cat**: put (put single token), move (move token up),
        square (finish square and remove 2 tokens from board)
    - **x**: x coordinate of token being put
    - **y**: y coordinate of token being put
    - **level**: level of token being put
    - prefix **take**: coordinates of token being moved (if cat == move)
        or coordinate of token being removed (if cat == square)
    - predix **take_sq**: coordinates of token being removed,
        if only one token is being removed *take_sq** coordinates should equal -1
    """
    ...
    def __init__(self, json_msg) -> None:
        ...
        if self.cat == "square":
            self.take_sq_x: int = json_msg[
                "take_sq_x"
            ]  # -1 indicates square move with only one token taken
            self.take_sq_y: int = json_msg[
                "take_sq_y"
            ]  # -1 indicates square move with only one token taken
            self.take_sq_level: int = json_msg[
                "take_sq_level"
            ]  # -1 indicates square move with only one token taken
