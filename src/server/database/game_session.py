...
class GameSession:
    ...
    async def _end_game(self):
        ...
        await self._broadcast(
            GameOverMsg(
                {
                    'winner_id': self._players_ids[winner],
                    'winner_name': self._players_names[winner],
                    'winner_tokens': self._tokens[winner]
                }
            )
        )
        for connection in self._connections:
    ...
    async def _update_state(self, move):
        self._turn += 1
        player = self._turn % 2
        cost = 0
        self._board = []  # TODO
    ...
    def _is_authorized_for_move(self, websocket: WebSocket, player_id) -> bool:
